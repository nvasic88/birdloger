<template>
  <div class="poaching-observations-import">
...
export default {
  name: 'nzPoachingObservationsImport',
  components: {
    NzUserAutocomplete,
    NzColumnsPicker
  },
  ...
  methods: {
    ...
    submit() {
      ...
      axios.post('/api/poaching-observation-imports', this.makeForm())
    ...
    checkStatus() {
      return axios.get(`/api/poaching-observation-imports/${this.currentImport.id}`).then(({ data }) => {
        this.currentImport = data
